...
inline Instance parse_instance(const Json& json) {
    ...
    for_each_json(json, "networkInterfaces", [&](const Json& network_interface) {
        if (result.internal_ip.empty())
            result.internal_ip = field(network_interface, "networkIP");
        for_each_json(network_interface, "accessConfigs", [&](const Json& config) {
