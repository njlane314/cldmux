// A small, dependency-free build tool for producing the distributable cldmux header.
...
bool is_safe_project_path(std::string_view path) {
    constexpr std::string_view prefix = "cldmux/";
...
std::optional<std::string> project_include(std::string_view line,
    ...
    const bool mentions_project_header =
        line.find("\"cldmux/") != std::string_view::npos ||
        line.find("<cldmux/") != std::string_view::npos;
    ...
    if (remainder.front() == '"') {
        ...
        if (closing == std::string_view::npos) {
            if (remainder.find("cldmux/") != std::string_view::npos) {
    ...
    if (remainder.front() == '<') {
        const std::size_t closing = remainder.find('>', 1);
        if (closing != std::string_view::npos &&
            starts_with(remainder.substr(1, closing - 1), "cldmux/")) {
...
public:
    ...
    amalgamation generate() {
        output_ = "// cldmux\n"
                  "// Generated from the private cldmux source fragments.\n"
                  "// Do not edit this file directly.\n"
                  "\n"
                  "#ifndef NJLANE314_CLDMUX_INCLUDED\n"
                  "#define NJLANE314_CLDMUX_INCLUDED\n"
                  "\n";

        emit(root_);
        output_ += "#endif // NJLANE314_CLDMUX_INCLUDED\n";
...
struct options {
    fs::path root = "include/cldmux/cldmux.hpp";
    fs::path include_root = "include";
    fs::path output = "cldmux";
